...
(function() {
    ...
    function removeGoogleAds() {
        ...
        adLabels.forEach(label => {
            if (label.textContent.trim() === 'Ad' || label.textContent.trim() === 'Sponsored') {
                // Check if this is inside a search panel - if so, only remove the ad link, not the container
                const searchPanel = label.closest('.search-panel-container, [class*="search-panel"], .auto-results-wrapper');
                if (searchPanel) {
                    // Only remove the specific ad link, not the whole container
                    const adLink = label.closest('a[class*="ads"], a[class*="-ad"]');
                    if (adLink) {
                        adLink.remove();
                        adsRemoved++;
                    }
                } else {
                    // For non-search contexts, remove the parent container as before
                    const parent = label.closest('.position-relative');
                    if (parent &&
                        !parent.querySelector('input[type="text"]') &&
                        !parent.querySelector('input[type="search"]')) {
                        parent.remove();
                        adsRemoved++;
                    }
                }
    ...
    function blockAdScripts() {
        // Block Google and Connatix ad scripts via MutationObserver
        const observer = new MutationObserver(function(mutations) {
            mutations.forEach(function(mutation) {
                mutation.addedNodes.forEach(function(node) {
                    if (node.tagName === 'SCRIPT') {
                        const src = node.src || '';
                        const isGoogleAd = src.includes('googlesyndication') || 
                            src.includes('googletagservices') ||
                            src.includes('doubleclick') ||
                            src.includes('adsbygoogle');
                        const isConnatix = src.includes('connatix.com') ||
                            src.includes('connatix.renderer') ||
                            node.hasAttribute('data-connatix-token');
                        if (isGoogleAd || isConnatix) {
                            console.log('Blocked ad script:', src || '[inline connatix]');
                            node.remove();
                        }
                    }
                });
...
// ── NEW AD TYPES (Connatix, Freestar, sticky banners, img_ybfq) ──────────────
(function() {
    'use strict';

    // ── Connatix global kill-switch ──────────────────────────────────────────
    // Override window.connatix and window.cnx before any player script can use
    // them. This kills initialization even if the CDN script loads.
    (function killConnatixGlobals() {
        const noop = () => {};
        ...
        try {
            Object.defineProperty(window, 'connatix', { get: () => noopObj, set: noop, configurable: false });
        } catch(e) {}
        try {
            Object.defineProperty(window, 'cnx', { get: () => noopObj, set: noop, configurable: false });
        } catch(e) {}
        try {
            Object.defineProperty(window, 'CNX', { get: () => noopObj, set: noop, configurable: false });
        } catch(e) {}
        try {
            Object.defineProperty(window, 'connatixPlayer', { get: () => noopObj, set: noop, configurable: false });
        } catch(e) {}
        // Block postMessage-based player communication
        const _origPostMessage = window.postMessage.bind(window);
        window.postMessage = function(msg, ...args) {
            if (typeof msg === 'string' && msg.includes('connatix')) return;
            if (typeof msg === 'object' && msg && JSON.stringify(msg).toLowerCase().includes('connatix')) return;
            return _origPostMessage(msg, ...args);
        };
    })();

    function removeNewAds() {
        let removed = 0;

        // 1. Connatix video ad player — broad multi-pattern sweep
        //    Covers: custom <cnx> elements, class-based containers,
        //    data-attribute embeds, iframes, and whalewisdom-specific patterns
        const cnxAds = document.querySelectorAll(
            'cnx.cnx-main-container, ' +
            '[class*="cnx-main-container"], ' +
            '[class*="cnx-player-wrapper"], ' +
            '[class*="cnx-video-container"], ' +
            '[class*="cnx-ui-ad"], ' +
            '[class*="cnx-ad-container"], ' +
            '[class*="cnx-ad-bid-slot"], ' +
            'cnx[class*="cnx-in-desktop"], ' +
            // id-based patterns (whalewisdom and similar sites)
            '[id*="cnx_"], [id*="cnxPlayer"], [id*="connatix"], ' +
            '[class*="connatix"], ' +
            // data-attribute embed markers
            '[data-connatix-token], ' +
            '[data-player-id][data-connatix], ' +
            // iframes pointing to any Connatix subdomain
            'iframe[src*="connatix.com"], ' +
            'iframe[src*="vid.connatix"], ' +
            'iframe[src*="cdn.connatix"], ' +
            'iframe[src*="player.connatix"], ' +
            // JWP Connatix post-merger player iframes
            'iframe[src*="jwplatform.com/players/"], ' +
            'iframe[src*="jwpcdn.com"]'
        );
        cnxAds.forEach(el => {
            // Walk up to the outermost cnx container
            const root = el.closest('cnx.cnx-main-container, [class*="cnx-main-container"], [id*="connatix"], [class*="connatix"]') || el;
            if (!root._cnxRemoved) {
                root._cnxRemoved = true;
                root.remove();
                removed++;
            }
        });

        // 1b. Catch orphaned Connatix script tags and remove their parent wrapper
        document.querySelectorAll('script[src*="connatix.com"], script[data-connatix-token]').forEach(el => {
            const wrapper = el.closest('div, section, aside') || el.parentElement;
        ...
        // 2. Freestar ad units (fs-iai divs and data-freestar-ad containers)
        const freestarAds = document.querySelectorAll(
            'div.fs-iai, ' +
            '[data-freestar-ad], ' +
            '[id*="freestar"], ' +
            '[class*="freestar"]'
        );
        freestarAds.forEach(el => { el.remove(); removed++; });

        // 3. Fixed bottom sticky banner ads (the anchor-bottom pattern)
        //    Identifies fixed-position bottom divs that contain ad iframes
        const fixedDivs = document.querySelectorAll(
            'div[style*="position:fixed"][style*="bottom:0"], ' +
            'div[style*="position: fixed"][style*="bottom: 0"]'
        );
        fixedDivs.forEach(el => {
            // Only remove if it contains an ad iframe or close button pattern
            const hasAdIframe = el.querySelector('iframe[enjgioijew], iframe[src*="ad"], iframe[width="970"]');
            const hasCloseBtn = el.querySelector('[id*="close"], [id*="Close"]');
            const hasAdScript = el.innerHTML.includes('anchorBottomPlaceHolder') ||
                                el.innerHTML.includes('img_dggnygsg');
            if (hasAdIframe || hasCloseBtn || hasAdScript) {
                el.remove(); removed++;
            }
        });

        // 4. img_ybfq / img_dggn obfuscated ad wrappers (div, span, or any tag)
        //    Target the outermost matching ancestor to remove entire ad tree in one shot
        const ybfqRoots = new Set();
        document.querySelectorAll('[id^="img_ybfq"], [id^="img_dggn"], [class^="img_ybfq"]')
            .forEach(el => {
                // Walk up to find the topmost img_ybfq/img_dggn ancestor
                let root = el;
                let parent = el.parentElement;
                while (parent && (
                    (parent.id && (parent.id.startsWith('img_ybfq') || parent.id.startsWith('img_dggn'))) ||
                    (parent.className && typeof parent.className === 'string' && parent.className.startsWith('img_ybfq'))
                )) {
                    root = parent;
        ...
        ybfqRoots.forEach(el => { el.remove(); removed++; });

        // 5. Iframes with the enjgioijew obfuscation attribute
        const obfuscatedIframes = document.querySelectorAll('iframe[enjgioijew]');
        obfuscatedIframes.forEach(el => { el.remove(); removed++; });

        if (removed > 0) console.log(`[AdBlocker] Removed ${removed} new-type ad(s)`);
        return removed > 0;
    }
    ...
    // Run on DOM ready and observe mutations
    function initNewAds() {
        removeNewAds();
        new MutationObserver(removeNewAds).observe(document.body || document.documentElement, {
            childList: true, subtree: true
        });
        setInterval(removeNewAds, 1500);
    }

    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initNewAds);
    } else {
        initNewAds();
    }
})();
